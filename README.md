# Unity Claude Template — 내 게임을 시작하는 바탕

**한국어** · [English](README.en.md)

> **이 리포지터리는 "내 유니티 게임을 Claude 와 함께 만들기 시작하는 빈 바탕"입니다.**
...
1. **채비** (20분쯤) — 유니티·Claude Desktop·이 템플릿 내려받기
2. **다리 두 개 놓기** (5분쯤) — Claude 가 내 파일과 유니티에 손을 뻗을 수 있도록
...
### 2-2. Claude Bridge 다리 놓기 (1분쯤)

**1)** 터미널을 열고 파이썬 꾸러미 하나를 깔아 주세요:

```bash
pip3 install mcp
```

**2)** Claude Desktop 설정 파일을 열어서 다리 정보를 적어 둡니다.
...
**3)** `mcpServers` 안에 `claude-bridge` 항목을 덧붙여 주세요:
...
```json
{
  "mcpServers": {
    ...
    "claude-bridge": {
      "command": "python3",
      "args": [
        "/Users/여러분이름/Documents/my-game/scripts/claude-bridge-mcp/server.py"
      ]
    }
  }
}
...
경로는 **템플릿을 풀어 놓은 자리**로 바꿔 주세요.

**4)** Claude Desktop 을 완전히 끄고(맥이라면 `Cmd + Q`) 다시 열어 주세요.

**5)** 새 대화창에서 살짝 점검해 봅니다:
...
Claude 에게 `/이름` 으로 부르는 작업 묶음이에요. 여러분이 굳이 부르지 않아도 에이전트가 알아서 쓰는 스킬은 **자동** 표시를 붙였습니다. 각 스킬이 해주는 일과 예시 말투를 함께 적어 뒀어요.
